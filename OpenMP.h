...
void calSum()
{
    ...
    int partial_sum[8] = {0}; // Array to hold the partial sums
#pragma omp parallel
    {
        int thread_id = omp_get_thread_num();
        int start_index = thread_id * workload;
        int end_index = (thread_id + 1) * workload;
...
void calMax()
{
    ...
    omp_set_num_threads(num_threads);
    int local_max[2] = {0}; // Array to hold local max values
#pragma omp parallel
    {
        int thread_id = omp_get_thread_num();
        int start_index = thread_id * workload;
        ...
        local_max[thread_id] = data[start_index]; // Initialize local max
        for (int i = start_index; i < end_index; ++i)
        {
            if (data[i] > local_max[thread_id])
            {
                local_max[thread_id] = data[i];
            }
        }
    }

    // Find the overall max from local_max
    max = local_max[0];
